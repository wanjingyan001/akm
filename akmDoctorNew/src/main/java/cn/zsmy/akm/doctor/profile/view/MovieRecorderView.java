...
public class MovieRecorderView extends LinearLayout implements OnErrorListener {
    ...
    public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mWidth = a.getInteger(R.styleable.MovieRecorderView_move_width, 320);// 默认320
        mHeight = a.getInteger(R.styleable.MovieRecorderView_move_height, 240);// 默认240

        isOpenCamera = a.getBoolean(
                R.styleable.MovieRecorderView_is_open_camera, true);// 默认打开
        mRecordMaxTime = a.getInteger(
                R.styleable.MovieRecorderView_record_max_time, 30);// 默认为10
...
//            setCameraParams();
    ...
    private void createRecordDir() {
        ...
        File sampleDir = new File(Environment.getExternalStorageDirectory()
                + File.separator + "akmdoctor/video/");//录制视频的保存地址
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        ...
        // 创建文件
        try {
            mVecordFile = File.createTempFile(DateUtils.getCurrentTime() + "_", ".mp4", vecordDir);// mp4格式的录制的视频文件
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void initRecord() throws IOException {
        ...
        mMediaRecorder.setOutputFormat(MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);// 视频输出格式
