...
import cn.zsmy.akm.doctor.utils.MyCountTime;
...
public class InputSecurityCodeFragment extends BaseFragment implements View.OnClickListener, TextWatcher {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.send_voice_security_code:
                ...
                switch (code){
                    case 0:
                        url = UrlHelper.SEND_REGCODE + phone + "&flag=" + Constants.SEND_VOICE_SECURITY_CODE+"&platform=doctor";
                        break;
                    case 1:
                        url = UrlHelper.FIND_MIMA_CODE_URL + phone + "&flag=" + Constants.SEND_VOICE_SECURITY_CODE+"&platform=doctor";
                        break;
                }
                getCode(url);
                break;
                    ...
                    if (replaceInputCode != null) {
                        switch (code) {
                            case Constants.KEY_FROM_REGISTER:
                                url = UrlHelper.VERIFICATION + phone + "&verifyCode=" + securityCode +"&platform=doctor";
                                break;
                            case Constants.KEY_FROM_LOGIN:
                                url = UrlHelper.RESET_PWD_VERIFICATION + phone + "&verifyCode=" + securityCode + "&platform=doctor";
                                break;
                        }
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        if (TextUtils.isEmpty(mInputCodeEdt.getText().toString())) {
            mInputBtn.setEnabled(false);
        } else if (mInputCodeEdt.getText().length()<4){
            mInputBtn.setEnabled(false);
        }else {
