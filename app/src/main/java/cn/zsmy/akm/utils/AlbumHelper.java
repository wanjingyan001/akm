...
public class AlbumHelper {
    ...
    HashMap<String, PhotoAlbum> bucketList = new HashMap<String, PhotoAlbum>();
                ...
                if (cur.getString(photoPathIndex)
                    ...
                    Log.d(TAG,
                            "出现了异常图片的地址：cur.getString(photoPathIndex).substring="
                                    + cur.getString(photoPathIndex).substring(cur.getString(photoPathIndex).lastIndexOf("/") + 1,
                                    cur.getString(photoPathIndex).lastIndexOf(".")));
                } else {
                    if (new File(path).length() > 50) {
                        PhotoAlbum bucket = bucketList.get(bucketId);
                        if (bucket == null) {
                            bucket = new PhotoAlbum();
                            bucketList.put(bucketId, bucket);
                            bucket.imageList = new ArrayList<ImageModule>();
                            bucket.bucketName = bucketName;
                        }
                        bucket.count++;
                        ImageModule imageModule = new ImageModule();
                        imageModule.imageId = _id;
                        imageModule.imagePath = path;
                        imageModule.thumbnailPath = thumbnailList.get(_id);
                        bucket.imageList.add(imageModule);
                    }
                }
            } while (cur.moveToNext());
        }

        Iterator<Entry<String, PhotoAlbum>> itr = bucketList.entrySet().iterator();
        while (itr.hasNext()) {
            Map.Entry<String, PhotoAlbum> entry = (Map.Entry<String, PhotoAlbum>) itr.next();
            PhotoAlbum bucket = entry.getValue();
            Log.d(TAG, entry.getKey() + ", " + bucket.bucketName + ", " + bucket.count + " ---------- ");
            for (int i = 0; i < bucket.imageList.size(); ++i) {
                ImageModule image = bucket.imageList.get(i);
     ...
     * @return
     */
    public List<PhotoAlbum> getImagesBucketList(boolean refresh) {
